/*******************************************************************************
 *   Filename:       bsp_sht2x.c
 *   Revised:        All copyrights reserved to Roger-WY.
 *   Revision:       v1.0
 *   Writer:	     Roger-WY.
 *
 *   Description:    STH2x温湿度传感器驱动模块
 *   Notes:
 *
...
void  calc_sth2x(float *p_humidity ,float *p_temperature);
...
//    }                                      // 测量错误
    ...
    while(GPIO_ReadInputDataBit(SHT_PORT,GPIO_Pin_SHT_DAT)) {
//        /***********************************************
//        * 描述：可以在此处做超时检测
//        */
//        i ++;
 ...
 * 名    称： calc_sth2x
 * 功    能： 初始化滴答定时器，清空所有软定时器
 * 入口参数： p_humidity      SHT2x采集到的相对湿度值(经过本函数的转换，该参数返回实际物理量的值)
 *            p_temperature   SHT2x采集到的温度值(经过本函数的转换，该参数返回实际物理量的值)
 * 出口参数： 无
 * 作　　者： Roger-WY.
 * 创建日期： 2015-06-25
 * 修    改：
...
//    t_C=t*0.01 - 39.60;                                    // 计算温度物理量值(SHT1x)
//    rh_lin=C3*rh*rh + C2*rh + C1;

    t_C             = t * 175.72 / 16384 - 46.85;       // 计算温度物理量值 (SHT2x)
        ...
        */
        if(error!=0) {
           s_connectionreset();                   //in case of an error: connection SHTCMD_RESET
            ...
            calc_sth2x(&humi_val.f,&temp_val.f);            //calculate humidity, temperature
